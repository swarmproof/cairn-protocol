...
/// @title ArbiterRegistry - Decentralized dispute resolution
...
contract ArbiterRegistry is IArbiterRegistry, ReentrancyGuard {
    // ═══════════════════════════════════════════════════════════════
    ...
    constructor(address _cairnCore, address _governance, address _feeRecipient) {
    ...
    /// @dev Allows anyone to set if cairnCore is zero (initial deployment), then governance only
    function setCairnCore(address _cairnCore) external {
        if (cairnCore != address(0) && msg.sender != governance) revert NotGovernance();
        cairnCore = _cairnCore;
    }
