...
contract FallbackPool is IFallbackPool, ReentrancyGuard {
    ...
    /// @notice ERC-7710 delegation registry
    IERC7710 public delegationRegistry;

    /// @notice Olas Mech adapter for external fallback agents
    ...
    constructor(
        ...
        // Olas adapter is optional - can be zero
    ...
    {
        // Step 1: Query internal pool candidates
        address[] storage internalCandidates = _taskTypeAgents[taskType];

        uint256 bestScore;
        uint256 requiredStake = getMinStake(escrowAmount);

        // Evaluate internal pool agents
        for (uint256 i = 0; i < internalCandidates.length; i++) {
            address candidate = internalCandidates[i];
        ...
        // Step 2: Query Olas Mech Marketplace (if adapter configured)
        if (address(olasMechAdapter) != address(0)) {
            try olasMechAdapter.queryAvailableMechs(taskType) returns (address[] memory olasMechs) {
                // Evaluate Olas mechs
                // Note: Olas mechs are pre-filtered by the adapter for eligibility
                // We give them a base score of 75 (good reputation assumed from Olas)
                // This can be adjusted based on actual Olas reputation data
                uint256 olasMechScore = 75;

                for (uint256 i = 0; i < olasMechs.length; i++) {
                    if (olasMechScore > bestScore) {
                        bestScore = olasMechScore;
                        bestAgent = olasMechs[i];
                    }
                }
            } catch {
                // Olas query failed - continue with internal pool only
            }
        }
    ...
    /// @notice Get agent reputation from ERC-8004 registry
    function _getReputation(address agent) internal view returns (uint256) {
        // If no registry set, default to 70 for backward compatibility
        if (address(reputationRegistry) == address(0)) {
            return 70;
        }

        // Query ERC-8004 registry for global reputation
