...
/// @title CairnCore - Main CAIRN Protocol Contract
/// @author CAIRN Protocol
/// @notice Manages the complete task lifecycle with failure recovery
...
contract CairnCore is ICairnCore, ReentrancyGuard, Pausable {
    // ═══════════════════════════════════════════════════════════════
    // CONSTANTS
    // ═══════════════════════════════════════════════════════════════
    ...
    /// @notice ERC-8183 escrow hook (optional)
    IERC8183 public escrowHook;
    ...
    function submitTask(
        ...
        // Call ERC-8183 hook if set
        if (address(escrowHook) != address(0)) {
            try escrowHook.onTaskSubmitted(taskId, primaryAgent, msg.value) {}
            catch {} // Hook failures don't block task creation
        }
    }
    ...
    function commitCheckpointBatch(
        ...
        // Call ERC-8183 hook if set
        if (address(escrowHook) != address(0)) {
            try escrowHook.onCheckpoint(taskId, latestCID) {}
            catch {} // Hook failures don't block checkpoints
        }
    }
        ...
        nonReentrant
    {
        ...
        // Call ERC-8183 hook if set (before settlement)
        if (address(escrowHook) != address(0)) {
            try escrowHook.onTaskCompleted(taskId, true) {}
            catch {} // Hook failures don't block completion
        }
    ...
    function _settleDispute(
        ...
        // Call ERC-8183 settlement hook if set (dispute case)
        if (address(escrowHook) != address(0)) {
            try escrowHook.onSettlement(taskId, primaryPayout + fallbackPayout, operatorRefund) {}
            catch {} // Hook failures don't block settlement
        }
    }
    ...
    function _refundOperator(bytes32 taskId) internal {
        ...
        // Call ERC-8183 settlement hook if set (refund case)
        if (address(escrowHook) != address(0)) {
            try escrowHook.onSettlement(taskId, 0, escrow) {}
            catch {} // Hook failures don't block settlement
        }
    }
    ...
    /// @notice Set ERC-8183 escrow hook contract
    /// @dev Hook is optional - can be set to zero to disable
