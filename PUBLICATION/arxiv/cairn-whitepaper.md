...
| T+185s | Heartbeat missed (last heartbeat at T+120s; T+185s > T+120s + 60s interval) → anyone calls `checkLiveness` | **FAILED** |
...
```json
{
  ...
  "escrow_split": {
    "original_agent": "0.00597 ETH",
    "fallback_agent": "0.00398 ETH",
...
Where:
- *k* = minimum records for 63% pattern confidence (estimated: *k* ≈ 30 per task type). The value *k* ≈ 30 follows from the sample size formula for a one-proportion *z*-test: *n* =\\ (*z*~α/2~)² × *p*(1−*p*) / *d*², where *p* = 0.5 (failure rate), *d* = 0.10 (precision), *α* = 0.05 → *n* = 1.96² × 0.25 / 0.01 ≈ 96 total records. Distributing 96 records across the three classes gives\\ ~32 per class on average, which we round to the conservative uniform per-class threshold *k* ≈ 30. (The scarcest class, LOGIC at ~20%, accumulates records\\ ~2× slower than this average — see the cold-start decomposition below.)
...
**Derivation of the 10% fallback stake.** The minimum stake must satisfy two simultaneous constraints. First, it must make zero-checkpoint failure *economically irrational* for any agent whose ex-ante success rate exceeds the admission floor — i.e., the expected loss from staking exceeds the expected gain from an unjustified acceptance. Combining the fallback acceptance condition (Proposition 2 in §7.5) with the slash mechanism: a fallback with success rate *s* faces expected gain `s · E_r · (c_f/c_total) · (1−f) − (1−s) · p_0 · stake`. For the admission floor *s* = 0.5 (reputation gate), zero-checkpoint failure probability\\ *p*~0~ ≈ 0.3 (empirical estimate from the simulation ground truth where failures cluster early), and the typical case `(c_f/c_total) ≈ 0.4` (fallback takes\\ ~40% of work on recovery), the slash-only break-even stake is approximately `0.5 × 0.4 × E_r / (0.5 × 0.3) ≈ 1.33 × E_r` — i.e., a stake exceeding the remaining escrow itself, which no practical per-task deposit can meet. The 10% figure is therefore *not* claimed to satisfy this bound on its own. CAIRN anchors the stake to the *maximum eligible escrow* across the agent's declared task-type portfolio (`stake = 0.1 × E_max`), so the slash alone deters unjustified acceptance for any recovery task with `E_r ≤ 0.075 × E_max` (i.e., the long tail of small tasks a fallback serves), and the remaining range is closed by the second deterrent that the slash triggers alongside it: a zero-checkpoint failure also decays the agent's ERC-8004 reputation below the admission floor (Section 4.4), forfeiting the agent's entire future fallback income stream — a repeated-game cost that dominates any single-task gain for a professionally operated fallback. The stake's role is to make Sybil re-entry costly (a fresh identity must re-post capital and re-earn reputation), not to outweigh a single large escrow by itself. Second, the stake must be small enough that capable agents will actually post it — 10% is large enough to deter Sybils but small enough that a reputable fallback can serve many tasks without locking excessive capital. Empirically, 10% is the floor used by similar staked-fallback designs (e.g., Olas's mech registration); CAIRN inherits this convention.

**Derivation of the 20% arbiter stake.** The arbiter stake must dominate the arbiter fee by a factor large enough to make collusion uneconomical. With fee φ = 3% of dispute value and slash amount 50% of stake on detection, the no-collusion condition is `bribe < φ + p_d × (slash/2 × stake) = 0.03V + p_d × stake/2`. Setting `stake = 0.20V` and `p_d = 0.8` (high-detection regime, on-chain evidence) yields `bribe < 0.03V + 0.08V = 0.11V` — the minimum bribe must exceed 11% of dispute value. For the LOGIC-dispute regime where *p*~d~ ≈ 0.4, the same stake yields `bribe < 0.03V + 0.04V = 0.07V` — the margin thins but does not collapse; this is the residual structural risk discussed in §7.5 and §10.3. A higher stake (e.g., 30%) would tighten the LOGIC-dispute bound but at the cost of arbiter participation — 20% is the compromise that keeps arbiter capital cost roughly equal to\\ ~6-7× the per-dispute fee, which we judge is the lowest acceptable risk-to-reward ratio that still attracts professional arbiters. The v1 testnet uses 15% pending the v2 upgrade.
...
**v2 formula (Equation 4 — simulation-validated, this paper's headline result).** Equation numbering throughout this paper follows the simulation's formula indices (Eq1 linear, Eq2 piecewise, Eq3 five-variable, Eq4 multiplicative; Section 10.1):
```
r =\\ F^a × B^b ×\\ D^c                                    (Eq. 4)
```
...
- **Total Eq4 misrouting cost:\\ ~0.302 ETH / 1,000 tasks (~$754 at $2,500/ETH, i.e.,\\ ~$0.75 per task).**
...
The commit-reveal scheme provides an additional layer: the arbiter commits a hash of their ruling before knowing which agents are involved (preventing pre-arrangement), and must reveal within 24 hours or forfeit their eligibility. **QED.**
...
| Recovery exponent *a* | 0.80 | 0.1-2.0 | Exponent for failure class weight in Equation 4 |
| Recovery exponent *b* | 0.35 | 0.1-2.0 | Exponent for budget remaining in Equation 4 |
| Recovery exponent *c* | 0.15 | 0.1-2.0 | Exponent for deadline remaining in Equation 4 |
...
**ERC-7710 (Delegation Framework) [17].** Enables scoped permission transfer. CAIRN uses ERC-7710 for pre-authorized fallback delegation: the operator grants CAIRN permission to sub-delegate task authority to a fallback agent at recovery time, without requiring a new operator signature.
...
- **Adjacent lines of work.** Blockchain-mediated incentive-compatible mechanisms for multi-agent reinforcement learning [4] and the broader agents-meet-blockchain survey literature [10] establish the design space CAIRN operates in but do not address mid-task failure recovery. In the networking domain, an IETF draft proposes a task-oriented multi-agent recovery framework for converged mobile networks [11] — conceptually parallel (checkpointed handoff between agents) but without economic settlement or permissionless enforcement; the convergence of independent efforts on agent recovery underscores that the gap CAIRN targets is recognized across communities.
