...
Researcher \textbf{Contact:} github.com/swarmproof
...
agent. The accumulated execution history grows with task throughput and
...
coordination protocols (Google A2A, Anthropic MCP). All simulation code,
results, and figures are reproducible from \texttt{simulation/} in the
CAIRN repository via \texttt{python3\ -m\ simulation.run\_eq4}
(seed=42); see reference {[}18{]}.
...
score (Equation 1 in Section 6.4) with binary routing, reflecting the
...
\item
  \protect\hyperlink{2-protocol-specification}{Protocol Specification}
\item
  \protect\hyperlink{3-design-philosophy}{Design Philosophy}
\item
  \protect\hyperlink{4-key-design-decisions}{Key Design Decisions}
\item
  \protect\hyperlink{5-execution-intelligence-layer}{Execution
  Intelligence Layer}
\item
  \protect\hyperlink{6-economic-model}{Economic Model}
\item
  \protect\hyperlink{7-security-model}{Security Model}
\item
  \protect\hyperlink{8-governance}{Governance}
\item
  \protect\hyperlink{9-related-work}{Related Work}
\item
  \protect\hyperlink{10-future-work}{Future Work}
\item
  \protect\hyperlink{11-references}{References}
\end{enumerate}
...
\item
  ...
  succeed only\textbackslash{} \textasciitilde20\% of the
  time\textbackslash{} (0.85\textsuperscript{10} = 0.197). Real
  production accuracy is lower, and the 50\% headline rate is the
...
\item
  Research on AI agent reliability finds that \textbf{outcome
  consistency remains low across all models} --- even as calibration has
  improved in recent models --- and that agents cannot reliably determine
  when they are likely to fail {[}2{]}. This validates the need for external
...
\hypertarget{protocol-specification}{%
...
(\emph{id}, \emph{σ}, \emph{A}\textsubscript{p},\textbackslash{}
\emph{A}\textsubscript{f}, \emph{E}, \emph{δ}, \emph{H},\textbackslash{}
...
\emph{D}\textsubscript{timeout}. \textbf{QED.}
...
new state to contract storage before function return. (Theorem 3 is
stated for the v2 state graph. The v1 deployment reaches RECOVERING →
DISPUTED indirectly by re-entering FAILED on fallback failure --- see
Section 7.4, invariant 4 --- so strict monotonicity over the FAILED
state holds in v2 but not v1; the reachable terminal set is identical in
both versions.) \textbf{QED.}
...
(Equation 4 in Section 6.4), and the v2 threshold comparisons (\emph{r}
...
\hypertarget{worked-example-the-247am-recovery}{%
...
T+185s & Heartbeat missed (last heartbeat at T+120s; T+185s
\textgreater{} T+120s + 60s interval) → anyone calls
\texttt{checkLiveness} & \textbf{FAILED} \\
...
resumes from step 4. Original agent paid 0.00597 ETH for verified work
(60\% of the 0.00995 ETH distributable after the 0.5\% protocol fee).
Total recovery time: \textasciitilde85 seconds.

\begin{quote}
\emph{Detection-window note.} The timeline above uses the strict
single-interval liveness window
(\texttt{block.timestamp\ \textgreater{}\ lastHeartbeat\ +\ H}), as
implemented in the MVP contract variant. The full v1 \texttt{CairnCore}
deployment applies the more conservative two-consecutive-miss rule of
Section 4.3 (\texttt{lastHeartbeat\ +\ 2H}), under which detection fires
at\textbackslash{} \textasciitilde T+245s --- 125 seconds after the
crash rather than 65. Either way, detection is measured in seconds, not
the hours typical of polling-based discovery.
\end{quote}

\hypertarget{comparative-analysis-recovery-vs.-restart}{%
...
\textbackslash{} \textasciitilde4h (manual) & 66\% (2/3 steps) & 0\% &
\textasciitilde90s total & Full re-execution \\
DeFi rebalance & 5 & Step 3 & \textbackslash{} \textasciitilde65s &
\textasciitilde4h & 60\% (3/5 steps) & 0\% & \textbackslash{}
\textasciitilde85s total & Full re-execution \\
Data pipeline & 10 & Step 7 & \textasciitilde65s & \textbackslash{}
\textasciitilde4h & 70\% (7/10 steps) & 0\% & \textasciitilde70s total &
Full re-execution \\
Long analysis & 20 & Step 15 & \textbackslash{} \textasciitilde65s &
\textasciitilde4h & 75\% (15/20 steps) & 0\% & \textbackslash{}
\textasciitilde65s total & Full re-execution \\
Complex pipeline & 50 & Step 42 & \textasciitilde65s & \textbackslash{}
...
\$0 & Steps 1-3:\textbackslash{} \textasciitilde\$0.003 \\
...
1-42: \$0 & Steps 1-42:\textbackslash{} \textasciitilde\$0.035 \\
\end{longtable}
...
\hypertarget{classify-by-recoverability-not-symptom}{%
...
partition occurred. These are almost always recoverable
...
window overflowed. These are partially recoverable\textbackslash{}
(\textasciitilde48\% base rate): success depends on whether sufficient
budget and deadline remain for the fallback.

\textbf{LOGIC failures} (\emph{F} = 0.00) --- the agent reasoned
incorrectly. Step repetition loops, hallucinated outputs, or
specification mismatches. These are rarely recoverable
...
\hypertarget{resume-not-restart}{%
...
\hypertarget{permissionless-enforcement}{%
...
\hypertarget{escrow-as-forcing-function}{%
...
\hypertarget{checkpoint-protocol}{%
...
\hypertarget{checkpoint-portability}{%
...
cover\textbackslash{} \textasciitilde80-90\% of current on-chain agent
workloads based on the task-type distribution observed in Olas and
Virtuals deployments. Reasoning-heavy workloads receive degraded
recovery rather than no recovery; quantifying the exact degradation
requires empirical study, which is reserved for v3 (Section 10.1, open
research questions).
\end{quote}
...
\subsection{4.1.2 Data Availability}\label{data-availability}}
...
\hypertarget{task-type-taxonomy}{%
...
\hypertarget{adaptive-liveness-interval}{%
...
\hypertarget{fallback-pool-admission-control}{%
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"escrow\_split"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"original\_agent"}\FunctionTok{:} \StringTok{"0.00597 ETH"}\FunctionTok{,}
    \DataTypeTok{"fallback\_agent"}\FunctionTok{:} \StringTok{"0.00398 ETH"}\FunctionTok{,}
...
\hypertarget{network-effects}{%
\subsection{5.3 Network Effects}\label{network-effects}}

The execution history is on-chain and publicly indexable, growing with
task throughput. As more agents complete tasks under CAIRN, the failure
patterns, agent performance data, and cost distributions accumulate into
a richer signal for every future routing and fallback-selection
decision. Each new agent failure makes the protocol more valuable for
every future agent.
...
0.01 ≈ 96 total records. Distributing 96 records across the three
classes gives\textbackslash{} \textasciitilde32 per class on average,
which we round to the conservative uniform per-class threshold \emph{k}
≈ 30. (The scarcest class, LOGIC at \textasciitilde20\%, accumulates
records\textbackslash{} \textasciitilde2× slower than this average ---
see the cold-start decomposition below.) - \emph{F}\textsubscript{0} =
baseline fallback success rate without intelligence (random selection
from pool) -\textbackslash{} \emph{F}\textsubscript{max} = maximum
fallback success rate with full intelligence - \emph{m} = records needed
for 63\% of maximum improvement (estimated: \emph{m} ≈ 100 per task
type)
...
\hypertarget{stake-requirements}{%
...
\textasciitilde40\% of work on recovery), the slash-only break-even
stake is approximately
\texttt{0.5\ ×\ 0.4\ ×\ E\_r\ /\ (0.5\ ×\ 0.3)\ ≈\ 1.33\ ×\ E\_r} ---
i.e., a stake exceeding the remaining escrow itself, which no practical
per-task deposit can meet. The 10\% figure is therefore \emph{not}
claimed to satisfy this bound on its own. CAIRN anchors the stake to the
\emph{maximum eligible escrow} across the agent's declared task-type
portfolio (\texttt{stake\ =\ 0.1\ ×\ E\_max}), so the slash alone deters
unjustified acceptance for any recovery task with
\texttt{E\_r\ ≤\ 0.075\ ×\ E\_max} (i.e., the long tail of small tasks a
fallback serves), and the remaining range is closed by the second
deterrent that the slash triggers alongside it: a zero-checkpoint
failure also decays the agent's ERC-8004 reputation below the admission
floor (Section 4.4), forfeiting the agent's entire future fallback
income stream --- a repeated-game cost that dominates any single-task
gain for a professionally operated fallback. The stake's role is to make
Sybil re-entry costly (a fresh identity must re-post capital and re-earn
reputation), not to outweigh a single large escrow by itself. Second,
the stake must be small enough that capable agents will actually post it
--- 10\% is large enough to deter Sybils but small enough that a
reputable fallback can serve many tasks without locking excessive
capital. Empirically, 10\% is the floor used by similar staked-fallback
designs (e.g., Olas's mech registration); CAIRN inherits this
convention.
...
\texttt{bribe\ \textless{}\ 0.03V\ +\ 0.04V\ =\ 0.07V} --- the margin
thins but does not collapse; this is the residual structural risk
discussed in §7.5 and §10.3. A higher stake (e.g., 30\%) would tighten
the LOGIC-dispute bound but at the cost of arbiter participation ---
...
\hypertarget{recovery-score-formula-v2-specification}{%
...
\textbf{v2 formula (Equation 4 --- simulation-validated, this paper's
headline result).} Equation numbering throughout this paper follows the
simulation's formula indices (Eq1 linear, Eq2 piecewise, Eq3
five-variable, Eq4 multiplicative; Section 10.1):
...
\begin{quote}
\textbf{v1 interim formula (Equation 1 --- current testnet deployment).}
The v1 contract on Base Sepolia implements the pre-calibration linear
formula \texttt{r\ =\ 0.5·F\ +\ 0.3·B\ +\ 0.2·D} (Eq. 1) with class
weights (0.90, 0.50, 0.10) and a single binary threshold at 0.30 --- the
47.56\%-misrouting baseline called ``Eq1-current'' in Section 10.1. The
calibration work in this paper motivates the v2 upgrade path: replace
the linear formula with the multiplicative formula above, move class
weights to (0.70, 0.30, 0.00), and introduce the three-tier threshold
band (0.40 / 0.35). The migration is governance-gated and does not
require a state-breaking upgrade, because the score formula is already
isolated behind the \texttt{IRecoveryRouter} interface in the v1
...
\hypertarget{gas-costs}{%
\subsection{6.5 Gas Costs}\label{gas-costs}}

All operations are designed for Base L2, where gas is inexpensive.
\textbf{Every row below is now measured} from
\texttt{forge\ test\ -\/-gas-report} against the full v2 reference
implementation (359-test suite passing on this branch); the raw report is
committed at \texttt{contracts/gas-report-v2-full.txt}. CairnCore rows
report the median of successful calls;
\texttt{computeRecoveryScore}/\texttt{classifyAndScore} report max/avg/min
across their input range.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.3768}}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{submitTask} (incl. fallback auto-selection) & 460,091 (median) &
4.6 × 10⁻⁶ ETH & \$0.0115 & measured \\
\texttt{commitCheckpointBatch} (any batch size) & 195,199 (median) & 2.0
× 10⁻⁶ ETH & \$0.0049 & measured \\
\texttt{heartbeat} & 31,220 (median) & 3.1 × 10⁻⁷ ETH & \$0.00078 &
measured \\
\texttt{completeTask} (settlement) & 148,921 (median) & 1.5 × 10⁻⁶ ETH &
\$0.0037 & measured \\
...
Two measured results are worth highlighting. First,
\textbf{\texttt{commitCheckpointBatch} gas is independent of checkpoint
count} --- because Merkle batching commits a single root per batch, an
isolated call costs \textasciitilde158,528 gas of execution whether it
commits 1, 10, or 50 checkpoints (benchmark in
\texttt{test/GasBenchmark.t.sol}). Batching therefore amortizes
checkpoint cost to near-zero per checkpoint at scale. Second,
\textbf{\texttt{submitTask} (\textasciitilde460k) is higher than earlier
design estimates (\textasciitilde180k)} because v2 auto-selects the
fallback agent from the pool via an external call rather than requiring a
pre-declared fallback --- a deliberate trade of gas for operator
convenience and a larger fallback set. Even so, at 0.01 gwei on Base the
full task lifecycle (submit + checkpoint + settle) costs well under two
cents.
...
\hypertarget{economic-impact-of-misrouting}{%
...
\item
  ...
  \textasciitilde0.302 ETH / 1,000 tasks (\textasciitilde\$754 at
  \$2,500/ETH, i.e.,\textbackslash{} \textasciitilde\$0.75 per task).}
\end{itemize}
...
\textbf{\textasciitilde0.452 ETH / 1,000 tasks\textbackslash{}
(\textasciitilde\$1,130)}, a \textbf{60\% reduction} in misrouting cost.
...
\hypertarget{attack-vectors-and-mitigations}{%
...
\hypertarget{protocol-invariants}{%
...
\hypertarget{incentive-analysis}{%
...
\hypertarget{game-1-checkpoint-commitment}{%
...
\bottomrule\noalign{}
\endlastfoot
5 & 0.001 ETH & 9.87 × 10⁻⁵ & Yes (Δ\emph{V} \textgreater\textgreater{}
\emph{g}) \\
...
into an on-chain invariant. \textbf{QED.}

\hypertarget{game-2-fallback-acceptance}{%
...
never reach the decision. \textbf{QED.}
...
\hypertarget{game-3-arbiter-ruling}{%
...
LOGIC cases at 0.3-0.5 rather than 0.8, which lowers the minimum bribe
bound to roughly 0.06\emph{V}-0.08\emph{V} (0.03\emph{V}
+\textbackslash{} \emph{p}\textsubscript{d} × 0.1\emph{V}) --- still
meaningful, but a much narrower margin than the 0.11\emph{V} of the
on-chain-evidenced classes.
...
\hypertarget{agent-frameworks-with-failure-handling}{%
...
\hypertarget{on-chain-agent-coordination}{%
...
\textbf{ERC-7710 (Delegation Framework) {[}17{]}.} Enables scoped
permission transfer. CAIRN uses ERC-7710 for pre-authorized fallback
delegation: the operator grants CAIRN permission to sub-delegate task
authority to a fallback agent at recovery time, without requiring a new
operator signature.
...
\hypertarget{off-chain-coordination-protocols}{%
...
\hypertarget{academic-foundations}{%
...
\item
  ...
  went wrong.
\item
  \textbf{Adjacent lines of work.} Blockchain-mediated
  incentive-compatible mechanisms for multi-agent reinforcement learning
  {[}4{]} and the broader agents-meet-blockchain survey literature
  {[}10{]} establish the design space CAIRN operates in but do not
  address mid-task failure recovery. In the networking domain, an IETF
  draft proposes a task-oriented multi-agent recovery framework for
  converged mobile networks {[}11{]} --- conceptually parallel
  (checkpointed handoff between agents) but without economic settlement
  or permissionless enforcement; the convergence of independent efforts
  on agent recovery underscores that the gap CAIRN targets is recognized
  across communities.
\end{itemize}
...
\hypertarget{open-research-questions}{%
\subsection{10.1 Open Research
Questions}\label{open-research-questions}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2955}}
...
\hypertarget{protocol-extensions}{%
...
standard. The specification in
\href{https://github.com/swarmproof/cairn-protocol/blob/main/ERC-CAIRN.md}{ERC-CAIRN.md}
