...
\textbf{Contact:} github.com/MarouaBoud
\end{quote}
...
We present CAIRN, the first protocol to classify agent failures by
\textbf{recoverability} rather than symptom, enabling deterministic
...
\emph{F}\textsuperscript{0.80} ×~\emph{B}\textsuperscript{0.35}
...
checkpoint, or to dispute resolution. The formula is empirically
validated via Monte Carlo simulation across 100,000 task-failure events
and 16 experiments, achieving 23.46\% misrouting --- within 0.93pp of
the Bayes-optimal theoretical minimum (22.53\%) --- and reducing
wasted-recovery false positives by 65\% versus a linear baseline. Escrow
is settled proportionally to verified work. We prove escrow safety,
...
agent. The accumulated execution history creates a network effect that
cannot be forked.
...
{[}5{]}, \textasciitilde49,000 registered agent identities via ERC-8004
across 30+ EVM chains (as of February 2026) {[}14{]}, and growing
on-chain commerce via ERC-8183 {[}15{]}. But every agent is
...
\item
  ...
  MetaGPT, and AutoGen {[}3{]}. At 85\% per-action accuracy (a commonly
  cited single-step reliability for current LLM agents), a 10-step
  workflow succeeds only~\textasciitilde20\% of the
  time~(0.85\textsuperscript{10} = 0.197).
\item
...
its escrow. The operator is the \emph{principal} on whose behalf the
work is performed. CAIRN is agnostic to what the operator actually is:
...
\bottomrule\noalign{}
\endlastfoot
Externally-owned account (human-controlled) & A developer using a wallet
to dispatch an agent task & Wallet signs \texttt{submitTask}
transaction \\
Agent address & A higher-level orchestration agent decomposing a goal
into CAIRN-managed subtasks & Agent's own wallet signs
\texttt{submitTask} \\
Smart-contract account & A DeFi protocol routinely dispatching
maintenance work & Contract calls \texttt{submitTask} from its execution
context \\
DAO multisig & A treasury that approves agent tasks via on-chain
governance & Multisig collectively signs \texttt{submitTask} \\
\end{longtable}
}

The ``no human required'' property of CAIRN is therefore precise: humans
may submit tasks if they choose, but \textbf{no human signature,
intervention, or polling is required at any point between task
submission and final settlement}. Failure detection, classification,
recovery routing, fallback execution, dispute initiation, and escrow
distribution all proceed via permissionless on-chain enforcement
(Section 3.3) regardless of who or what the operator is.
...
(\emph{id}, \emph{σ},
\emph{A}\textsubscript{p},~\emph{A}\textsubscript{f}, \emph{E},
\emph{δ},
\emph{H},~\emph{c}\textsubscript{p},~\emph{c}\textsubscript{f},
\emph{κ},~\emph{t}\textsubscript{0}) where:
...
DISPUTED, RESOLVED\} \\
~\emph{A}\textsubscript{p},~\emph{A}\textsubscript{f} & address &
Primary and fallback agent addresses \\
\emph{E} & uint256 & Escrow amount in wei, \emph{E}
≥~\emph{E}\textsubscript{min} =~10\textsuperscript{15} (0.001 ETH) \\
\emph{δ} & uint256 & Deadline as block timestamp \\
\emph{H} & uint256 & Heartbeat interval in seconds, 30 ≤ \emph{H} ≤
\emph{δ}/4 \\
~\emph{c}\textsubscript{p},~\emph{c}\textsubscript{f} & uint256 &
Checkpoint counts for primary and fallback agents \\
\emph{κ} & uint256 & Cost accrued in wei \\
~\emph{t}\textsubscript{0} & uint256 & Task start block timestamp \\
\end{longtable}
}
...
DISPUTED & Timeout & block.timestamp ≥~\emph{t}\textsubscript{dispute}
...
RESOLVED within at most δ − t₀ +~D\textsubscript{timeout} seconds.}
...
−~\emph{t}\textsubscript{0}. - \textbf{RUNNING}: Either completes (→
RESOLVED) or a fault triggers (heartbeat miss at \emph{t} \textgreater{}
lastHeartbeat + \emph{H}, or deadline \emph{δ} reached). Maximum
duration: \emph{δ} −~\emph{t}\textsubscript{0}. - \textbf{FAILED}:
Recovery score \emph{r} is computed as a pure function of on-chain
state. Routing is immediate within the same transaction. Duration: 0. -
\textbf{RECOVERING}: Fallback either completes (→ RESOLVED) or fails (→
DISPUTED). Bounded by remaining deadline: \emph{δ}
−~\emph{t}\textsubscript{current}. - \textbf{DISPUTED}: Arbiter rules (→
RESOLVED) or timeout expires (→ RESOLVED with auto-refund). Bounded
by~\emph{D}\textsubscript{timeout} (default 604,800 seconds = 7 days).

RUNNING and RECOVERING share the same time
window~{[}\emph{t}\textsubscript{0}, \emph{δ}{]} --- RECOVERING begins
at~\emph{t}\textsubscript{fail} ≥~\emph{t}\textsubscript{0} and can only
consume time up to \emph{δ}, so the two intervals are not additive. The
worst-case path therefore takes at most (\emph{δ}
−~\emph{t}\textsubscript{0}) seconds before DISPUTED is entered, plus at
most~\emph{D}\textsubscript{timeout} seconds in DISPUTED. Total upper
bound: (\emph{δ} −~\emph{t}\textsubscript{0})
...
T+186s & Recovery score =~\emph{F}\^{}0.80 × \emph{B}\^{}0.35
×~\emph{D}\^{}0.15 = 0.70\^{}0.80 ×~0.85\^{}0.35 × 0.88\^{}0.15 =
\textbf{0.752 × 0.945 × 0.981 = 0.697} & FAILED \\
...
\bottomrule\noalign{}
\endlastfoot
Simple API call & 3 & Step 2 & ~\textasciitilde65s (heartbeat) &
\textasciitilde4h (manual) & 66\% (2/3 steps) & 0\% &
~\textasciitilde90s total & Full re-execution \\
DeFi rebalance & 5 & Step 3 & \textasciitilde65s & ~\textasciitilde4h &
60\% (3/5 steps) & 0\% & \textasciitilde85s total & Full re-execution \\
Data pipeline & 10 & Step 7 & ~\textasciitilde65s & \textasciitilde4h &
70\% (7/10 steps) & 0\% & ~\textasciitilde70s total & Full
re-execution \\
Long analysis & 20 & Step 15 & \textasciitilde65s & ~\textasciitilde4h &
75\% (15/20 steps) & 0\% & \textasciitilde65s total & Full
re-execution \\
Complex pipeline & 50 & Step 42 & ~\textasciitilde65s &
\textasciitilde4h & 84\% (42/50 steps) & 0\% & ~\textasciitilde60s total
& Full re-execution \\
\end{longtable}
}
...
\$0 & Steps 1-7:~\textasciitilde\$0.006 \\
...
partition occurred. These are almost always
recoverable~(\textasciitilde92\% base rate): the task is not impossible,
the agent simply died. A fallback can resume from the last checkpoint
immediately.
...
specification mismatches. These are rarely
recoverable~(\textasciitilde8\% base rate): a fallback with the same
task specification will likely fail the same way. Setting \emph{F} =
0.00 routes all LOGIC failures directly to dispute.
...
\textbf{Definition (Semantic Portability).} A checkpoint
\emph{C}\textsubscript{i} for subtask \emph{i} is \emph{semantically
portable} from framework~\emph{F}\textsubscript{1} to
framework~\emph{F}\textsubscript{2} if and only if an agent
running~\emph{F}\textsubscript{2} can produce a correct output for
subtask \emph{i+1} given only~\emph{C}\textsubscript{i} and the task
specification \emph{S}, without access to~\emph{F}\textsubscript{1}'s
internal state.
...
Olas's~\textasciitilde2,000 deployed mechs (≈500 active daily), subject
to integration of CAIRN's checkpoint schema with the Olas execution
model.
...
fee earned (3\%), making collusion uneconomical.
...
\emph{n} =~(\emph{z}\textsubscript{α/2})² × \emph{p}(1−\emph{p}) /
\emph{d}², where \emph{p} = 0.5 (failure rate), \emph{d} = 0.10
(precision), \emph{α} = 0.05 → \emph{n} = 1.96² × 0.25 / 0.01 ≈ 96 total
records. With a 45\% LIVENESS proportion, we need~\textasciitilde96 ×
0.45 ≈ 43 LIVENESS records --- or roughly 30 records per class when
amortized across the three classes. - \emph{F}\textsubscript{0} =
baseline fallback success rate without intelligence (random selection
from pool) -~\emph{F}\textsubscript{max} = maximum fallback success rate
with full intelligence - \emph{m} = records needed for 63\% of maximum
improvement (estimated: \emph{m} ≈ 100 per task type)
...
\emph{k}, confidence is 1 −~\emph{e}\textsuperscript{−1} ≈ 0.63. The
model predicts diminishing returns --- the first 100 records per task
type provide the majority of intelligence value, making the cold-start
problem bounded rather than open-ended.
...
operation) produces~\textasciitilde50 failure records per type ---
sufficient for 81\% pattern confidence (1 −~e\textsuperscript{−50/30}).
Reaching \emph{k} = 30 records per type (63\% confidence) takes
\textasciitilde36 days at this rate. When decomposed per failure class,
the LOGIC path saturates last~(\textasciitilde180 days to \emph{k} = 30
LOGIC records per type), but this does not block protocol utility:
LOGIC's recovery score is 0 by construction (Section 6.4), so it routes
to dispute without needing pattern confidence for the recover/dispute
...
2,646 grid points). The first three formulas converge to
a~\textasciitilde33\% misrouting floor --- a structural ceiling
intrinsic to additive formulas, confirmed across 3,008 configurations.
The multiplicative formula breaks through to 23.46\%, within
\textbf{0.93 percentage points of the Bayes-optimal theoretical minimum
(22.53\%)} --- capturing 96\% of achievable improvement. A hybrid
α-sweep (11 ratios from α=0.0 pure multiplicative to α=1.0 pure linear)
confirms that pure multiplicative is strictly optimal: misrouting
increases monotonically with α (23.46\% at α=0, 24.57\% at α=0.1,
26.27\% at α=0.5, 35.07\% at α=1.0). Cross-task-type leave-one-out
validation shows 23.39\% ± 0.36\% across five task types --- best
generalization of any formula tested. Most importantly, the confusion
matrix pivots: \textbf{FULL-tier false positives drop from 22.3\% (Eq1
linear) to 7.9\% (Eq4 multiplicative) --- a 65\% reduction in wasted
recovery attempts.} Full methodology, per-experiment findings, and
confusion matrices are documented in Section 10.1; raw results in
...
produces~\emph{F}\^{}0.80 ≈ 0.752, while a RESOURCE failure (\emph{F} =
0.30) produces \emph{F}\^{}0.80 ≈ 0.382 --- roughly a 2× separation
(precise ratio 1.97). The sub-linear exponent provides diminishing
returns above \emph{F} = 0.5, preventing the class signal from
overwhelming resource signals. The budget exponent \emph{b} = 0.35
assigns moderate influence: 50\% budget remaining
yields~\emph{B}\^{}0.35 = 0.79, while 10\% yields \emph{B}\^{}0.35 =
0.47 --- a meaningful but not catastrophic penalty. The deadline
exponent \emph{c} = 0.15 assigns the least weight: in the multiplicative
context, deadline contributes through the product interaction (low
deadline × low budget is catastrophic) more than through its individual
exponent. All exponents are governance-adjustable parameters (see
Section 8).

\textbf{Class weight rationale.} LIVENESS failures (agent crashes, API
timeouts) have the highest base recovery rate (\textasciitilde92\% when
resources are available), justifying~\emph{F}\textsubscript{LIVENESS} =
0.70. RESOURCE failures (budget exhaustion, context overflow) are
partially recoverable~(\textasciitilde48\%), justifying
\emph{F}\textsubscript{RESOURCE} = 0.30. LOGIC failures (reasoning
errors, hallucinations, spec mismatches) have~\textasciitilde8\% base
recovery rate --- a different agent retrying the same reasoning task
rarely succeeds. Setting \emph{F}\textsubscript{LOGIC} = 0.00 routes all
LOGIC failures directly to dispute, which is the economically correct
decision: the expected value of a recovery attempt (8\% × escrow saved)
is less than the expected cost (92\% × wasted fallback budget).

\textbf{Threshold rationale.} The upper threshold of 0.40 and lower
threshold of 0.35 create a narrow routing band. This reflects the
simulation finding that the multiplicative formula's primary value is in
the binary recover/dispute decision, not in distinguishing full from
reduced scope. LIVENESS failures with any resource headroom score well
above 0.40 (e.g., \emph{F}=0.70, \emph{B}=0.85, \emph{D}=0.88 →
\emph{r}=0.697). RESOURCE failures score in the 0.25-0.45 range
depending on resources. LOGIC failures score 0 regardless. All
thresholds are governance-adjustable parameters (see Section 8).
...
(0.70, 0.30, 0.00), the~\emph{F}\^{}\emph{a} term is implemented as a
lookup (3 pre-computed values), eliminating the most expensive
\texttt{pow} call. The remaining \emph{B}\^{}\emph{b}
and~\emph{D}\^{}\emph{c} terms use PRBMath's \texttt{pow}
function~(\textasciitilde3,000 gas each) or a binned lookup table for
further gas savings. Total cost: \textasciitilde2,500-6,200 gas
depending on implementation --- negligible on Base L2 (see Section 6.5).
...
\bottomrule\noalign{}
\endlastfoot
\texttt{submitTask} & ~\textasciitilde180,000 (estimate) & 1.8 × 10⁻⁶
ETH & \$0.0045 & design target \\
\texttt{commitCheckpointBatch} (1 checkpoint, v2) &
\textasciitilde80,000 (estimate) & 8.0 × 10⁻⁷ ETH & \$0.0020 & design
target \\
\texttt{commitCheckpointBatch} (10 checkpoints, v2) &
...
\texttt{settle} (escrow distribution) & ~\textasciitilde140,000
(estimate) & 1.4 × 10⁻⁶ ETH & \$0.0035 & design target \\
...
gas} is \emph{better} than the design-target
estimate~(\textasciitilde6,200 gas) used in earlier drafts. The maximum
observed (\textasciitilde20,000 gas) corresponds to the full
multiplicative path with two PRBMath UD60x18 \texttt{pow} calls; the
median~(\textasciitilde1,348 gas) is dominated by short-circuit paths
(LOGIC class returning 0, or zero-budget early termination --- see
...
(for~\emph{B}\^{}0.35 and \emph{D}\^{}0.15) account for most of the gas.
...
~\textasciitilde50\% of remaining escrow & 79 × 0.50 ×
\emph{E}\textsubscript{rem} \\
False positive --- REDUCED tier (capped budget wasted) & 5.4\% & 3.8\% &
~\textasciitilde25\% of remaining escrow & 54 × 0.25 ×
\emph{E}\textsubscript{rem} \\
...
remaining-budget~\emph{E}\textsubscript{rem} = 0.005 ETH at failure (so
per 1,000 tasks, each percentage-point of joint probability corresponds
to 10 events):
...
\item
  \textbf{Total Eq4 misrouting cost:~\textasciitilde0.303 ETH / 1,000
  tasks (\textasciitilde\$758 at \$2,500/ETH,
  i.e.,~\textasciitilde\$0.76 per task).}
\end{itemize}
...
\item
  ...
  tasks~(\textasciitilde\$1,885).}
\end{itemize}

The multiplicative v2 formula therefore saves
\textbf{\textasciitilde0.451 ETH / 1,000
tasks~(\textasciitilde\$1,128)}, a \textbf{60\% reduction} in misrouting
cost. The residual 0.93pp gap above the Bayes-optimal floor (22.53\%)
bounds the maximum further savings from calibration at roughly 0.012 ETH
...
at \emph{E} = 0.01): Eq4 misrouting costs~\textbf{\textasciitilde3.0\%
of escrow value}; Eq1-current costs \textasciitilde7.5\%. Eq4 reduces
the friction on deployed escrow by~\textasciitilde2.5× --- an acceptable
overhead for permissionless, trustless automated recovery.
...
time is consistent (\textasciitilde2s on Base) - Block builders on L2
(Base uses a sequencer) could theoretically reorder transactions to
front-run \texttt{checkLiveness}, but the atomic execution of failure
detection → classification → routing within a single transaction limits
the MEV surface to transaction ordering only, not mid-transaction state
insertion.
...
\item
  ...
  modified or deleted.
\item
  \textbf{State irreversibility.} No backward state transitions.
...
escrow E, n subtasks, empirical failure probability~p\textsubscript{f},
and per-checkpoint gas cost g, honest checkpointing is the dominant
strategy when~p\textsubscript{f} × E × (1 − f) / n \textgreater{} g,
where f is the protocol fee rate.}

\emph{Proof.} Agent \emph{A} executes task \emph{T} with \emph{n}
subtasks. At each subtask \emph{i}, \emph{A} chooses
action~\emph{a}\textsubscript{i} ∈ \{commit, skip\}. Let \emph{c} =
number of committed checkpoints. Let \emph{g} = gas cost per checkpoint
on Base L2 ≈ 8 × 10⁻⁷ ETH (80,000 gas × 0.01 gwei).
...
In case of success (probability 1 −~\emph{p}\textsubscript{f}), an extra
checkpoint costs \emph{g} but provides no additional payment. In case of
failure (probability~\emph{p}\textsubscript{f}), an extra checkpoint
increases the agent's share by \emph{E}(1 − \emph{f})/\emph{n} at cost
\emph{g}.

Checkpointing is dominant when Δ\emph{V} \textgreater{} 0. Evaluating
with protocol parameters (\emph{f} = 0.005, \emph{g} = 8 × 10⁻⁷ ETH) and
the empirical failure rate~\emph{p}\textsubscript{f} = 0.5 from {[}3{]}:
...
escrow for dominance is~\emph{E}\textsubscript{min} = (\emph{g} ×
\emph{n}) /~(\emph{p}\textsubscript{f} × (1 − \emph{f})) --- for
\emph{n} = 100:~\emph{E}\textsubscript{min} ≈ 1.6 × 10⁻⁴ ETH, well below
the protocol minimum of 10⁻³ ETH.
...
agent with success rate~s\textsubscript{τ} for task type τ should accept
a recovery assignment when the expected payoff exceeds zero:}

\begin{verbatim}
s_τ × E_r × (c_f / c_total) × (1 − f) > (1 − s_τ) × p_0 × stake
\end{verbatim}

\emph{where~E\textsubscript{r} is remaining
escrow,~c\textsubscript{f}/c\textsubscript{total} is the fallback's
expected checkpoint share, and~p\textsubscript{0} is the probability of
zero-checkpoint failure (triggering 100\% slash).}

\emph{Proof.} A fallback agent accepting an assignment faces a binary
outcome: - With probability~\emph{s}\textsubscript{τ} (success), the
fallback earns its share of remaining escrow after the protocol fee:
gain =~\emph{E}\textsubscript{r} ×~(\emph{c}\textsubscript{f}
/~\emph{c}\textsubscript{total}) × (1 − \emph{f}). - With probability (1
−~\emph{s}\textsubscript{τ}) (failure), the fallback either (i)
completes at least one checkpoint and earns partial payment without
stake loss, or (ii) fails without any checkpoint and is slashed 100\% of
stake.

Let~\emph{p}\textsubscript{0} denote the conditional probability of
zero-checkpoint failure given failure --- the sole slash trigger.
Expected payoff:
...
stated inequality. Three remarks: (a) at~\emph{s}\textsubscript{τ} = 1
the condition reduces to a trivially positive gain, recovering the
intuition that a perfectly competent fallback always accepts; (b) the
fallback's own gas cost \emph{g} and opportunity cost are absorbed
into~\emph{E}\textsubscript{r} for brevity (subtract them from the left
side to obtain the stricter form); (c) the admission gates (reputation ≥
50, stake ≥ 10\%) enforce a lower bound on~\emph{s}\textsubscript{τ} by
...
detection probability~p\textsubscript{d} for incorrect rulings, honest
ruling is the rational strategy when:}
...
payoff:} +bribe −~\emph{p}\textsubscript{d} × 0.5\emph{s} = bribe
−~\emph{p}\textsubscript{d} × 0.1\emph{V}

For dishonesty to be rational, the bribe must exceed the honest fee plus
the expected penalty: bribe \textgreater{} 0.03\emph{V}
+~\emph{p}\textsubscript{d} × 0.1\emph{V}.

The detection probability~\emph{p}\textsubscript{d} is high in CAIRN
because all evidence is on-chain: checkpoint CIDs, failure records,
heartbeat timestamps, and cost accruals are publicly verifiable.
With~\emph{p}\textsubscript{d} ≥ 0.8 (conservative --- most disputes
have clear on-chain evidence), the minimum bribe is 0.03\emph{V} +
0.08\emph{V} = 0.11\emph{V}.

\textbf{Multi-dispute analysis:} An arbiter who colludes in \emph{k}
disputes has cumulative detection probability 1 − (1 −\textbackslash{}
\emph{p}\textsubscript{d})\textsuperscript{k}.
At~\emph{p}\textsubscript{d} = 0.8: a single dispute has 80\% detection
risk; two disputes have 96\%; three disputes have 99.2\%. This makes
sustained collusion economically irrational --- the expected losses from
detection compound exponentially while the gains are linear.
...
remaining in Equation 1 \\
~\emph{F}\textsubscript{LIVENESS} & 0.70 & 0-1.0 & Failure class weight
for LIVENESS failures \\
~\emph{F}\textsubscript{RESOURCE} & 0.30 & 0-1.0 & Failure class weight
for RESOURCE failures \\
~\emph{F}\textsubscript{LOGIC} & 0.00 & 0-1.0 & Failure class weight for
LOGIC failures \\
\end{longtable}
}
...
with~\textasciitilde49,000 registered agents across 30+ EVM chains as of
February 2026 {[}14{]}. CAIRN reads reputation scores for fallback pool
admission and writes outcome signals (success/failure attestations)
...
\bottomrule\noalign{}
\endlastfoot
1 & \emph{r} = \emph{w}\textsubscript{f}\emph{F}
+~\emph{w}\textsubscript{b}\emph{B} +~\emph{w}\textsubscript{d}\emph{D}
& Linear & 3 (\emph{F}, \emph{B}, \emph{D}) & 47.56\% (current) →
\textbf{33.81\%} (optimized) \\
2 & Run 1 + piecewise cliffs
+~\emph{w}\textsubscript{int}·\emph{B}·\emph{D} interaction & Linear +
non-linear & 3 + 4 cliff params + 1 interaction & \textbf{33.17\%} \\
3 & Run 1 +~\emph{w}\textsubscript{c}·\emph{C} (complexity)
+~\emph{w}\textsubscript{s}·\emph{S} (fallback skill) & Linear & 5 (adds
\emph{C}, \emph{S}) & \textbf{32.78\%} \\
\textbf{4} & \textbf{\emph{r} =~\emph{F}\^{}\emph{a} ×
\emph{B}\^{}\emph{b} ×~\emph{D}\^{}\emph{c}} & \textbf{Multiplicative} &
\textbf{3 (\emph{F}, \emph{B}, \emph{D})} & \textbf{23.46\%} \\
...
a~\textasciitilde33\% misrouting structural ceiling. The
...
\emph{(w\textsubscript{f},~w\textsubscript{b},~w\textsubscript{d})}
simplex points & Current weights rank \#31/55; deadline should
dominate~(\emph{w\textsubscript{d}} ≥ 0.40) & −5.35pp \\
2 & 1 & Class weight optimization & Grid search over
245~\emph{(F\textsubscript{L},~F\textsubscript{R},~F\textsubscript{Log})}
combinations & ~\emph{F}\textsubscript{LOGIC} → 0.00 routes
...
~\emph{w\textsubscript{f}} most sensitive (+4.56pp at +20\%); stable
within ±10\% & --- \\
...
(\emph{w},~\emph{b}\textsubscript{crit},~\emph{d}\textsubscript{crit},
penalties,~\emph{w}\textsubscript{int}) &
Optimal:~\emph{b}\textsubscript{crit}=0.10,~\emph{d}\textsubscript{crit}=0.05,~\emph{w}\textsubscript{int}=0.25
& −0.64pp \\
...
with~\emph{w\textsubscript{c}},~\emph{w\textsubscript{s}} added &
Optimizer assigns minimum (0.05) to both new variables; at Eq1
thresholds the 5-var weight change alone produces 34.98\% (+1.17pp vs
Eq1-opt). Improvement arrives only when re-tuned thresholds (Exp 10) are
applied. & +1.17pp at Eq1 thresholds \\
...
0.36\% --- generalizes; confirms~\textasciitilde33\% ceiling is
...
RESOURCE class frequency & ~\textasciitilde35\% & 34.87\% & 0.13pp \\
LOGIC class frequency & \textasciitilde20\% & 20.17\% & 0.17pp \\
LIVENESS recovery at high resources & ~\textasciitilde92\% & 71.4\% &
Base rate × complexity/skill factors \\
RESOURCE recovery at high resources & \textasciitilde48\% & 37.9\% &
Same modulation \\
LOGIC recovery at high resources & ~\textasciitilde8\% & 5.4\% & Same
modulation \\
...
per task type~(\textasciitilde{} 60 days post-launch at 10 tasks/day,
per Section 5.3) --- \textbf{planned}; \textbf{(Stage 3)} full MCMC
re-calibration at mainnet with per-type posteriors --- \textbf{planned}.
At each stage the exponents are updated via governance parameter change,
not a code redeployment.
...
\textbf{Arbiter depth.} The current arbiter mechanism is single-tier
with no appeals. High-value disputes may require additional dispute
resolution infrastructure in future versions.
...
deployed, \textasciitilde500 active daily.
...
(55 weight + 245 class-weight + 62 threshold), runtime \textasciitilde3
seconds. Run 2: staged grid over 8 Eq2 parameters,
...
hybrid α-sweep, runtime \textasciitilde14 seconds. Reproducible:
\texttt{python3\ -m\ simulation.run} (Run 1), \texttt{run\_eq2} (Run 2),
\texttt{run\_eq3} (Run 3), \texttt{run\_eq4} (Run 4); seed=42,
deterministic on any NumPy ≥1.20 installation. Source:
